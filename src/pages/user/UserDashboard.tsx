...
const upcomingTrips = [
  { id: 1, route: "Aqaleem → Stadium", bus: "BUS-12", pickup: "Al-Rawda Square", time: "7:30 AM", date: "Tomorrow, Sun", status: "confirmed" as const, returnTime: "3:30 PM" },
  { id: 2, route: "Aqaleem → Stadium", bus: "BUS-12", pickup: "Al-Rawda Square", time: "7:30 AM", date: "Mon, Feb 24", status: "confirmed" as const, returnTime: "7:00 PM" },
];
...
const UserDashboard = () => {
  ...
  return (
    <UserLayout title="Dashboard" subtitle="Welcome back, Sara">
      <div className="space-y-6 max-w-4xl">
          ...
          <div className="glass-card p-6 relative overflow-hidden">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-base font-bold text-foreground">{nextTrip.route}</span>
              </div>
            ...
            <Button variant="glow" size="lg" className="w-full mt-4" onClick={() => navigate("/user/track")}>
              <Navigation className="h-4 w-4" /> Track Bus
            </Button>
          </div>
        </motion.div>

        {/* Quick Actions */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }} className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: "Book Trip", desc: "Register now", icon: Calendar, color: "text-primary", path: "/user/book" },
            { label: "My Trips", desc: "View all", icon: BookOpen, color: "text-primary", path: "/user/trips" },
            { label: "Route Chat", desc: "Group", icon: MessageCircle, color: "text-accent", path: "/user/chat" },
            { label: "Alerts", desc: "2 new", icon: Bell, color: "text-coral", path: "/user/notifications" },
          ].map((item) => (
            <button key={item.label} onClick={() => navigate(item.path)} className="glass-card-solid group p-4 text-left transition-all duration-200 hover:border-primary/20">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2 }}>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => navigate("/user/trips")} className="flex items-center gap-1 text-xs font-medium text-primary hover:underline">
              View All <ChevronRight className="h-3 w-3" />
            </button>
          </div>
          <div className="space-y-2">
            {upcomingTrips.map((trip) => (
              <div key={trip.id} className="glass-card-solid p-4 flex items-center justify-between transition-all duration-200 hover:border-primary/15">
        ...
        {/* Registration Window */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.3 }} className="glass-card-solid p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">Opens 12:00 AM — Closes 2:00 PM daily</p>
            </div>
          </div>
        </motion.div>
      </div>
